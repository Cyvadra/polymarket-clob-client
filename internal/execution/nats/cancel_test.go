...
func TestSubscribeCloseDecodesAndDispatches(t *testing.T) {
	execution, err := executor.New(fakeStore{positions: []store.PositionRecord{{ConditionID: "condition", TokenID: "token", Outcome: "Up", PositionSize: "1", ActualShares: "1", AvailableSize: "1", State: "open"}}}, fakeCLOB{}, time.Now)
	if err != nil {
	...
	request := protocol.ExecutionCloseRequest{SchemaVersion: protocol.SchemaVersionV1, Strategy: "strategy", ConditionID: "condition", AssetID: "token", Outcome: "Up", Mode: protocol.ExecutionCloseModeForce}
	...
	if len(publisher.values) != 1 || publisher.values[0].Status != protocol.ResultSucceeded || publisher.values[0].ConditionID != "condition" || publisher.values[0].AssetID != "token" {
