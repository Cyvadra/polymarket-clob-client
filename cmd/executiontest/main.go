...
func run() error {
	...
	flag.DurationVar(&config.CaseTimeout, "case-timeout", 5*time.Minute, "maximum time to wait for an OPEN or CLOSE terminal result")
	...
	flag.BoolVar(&config.Negative, "negative", true, "run the invalid-schema rejection probe before the lifecycle")
	flag.Parse()
